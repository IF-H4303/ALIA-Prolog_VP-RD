%TODO : ne pas compter plusieurs fois les pions solitaires.
evaluer(Grille, 999) :- victoireJoueur(x, Grille), !.
evaluer(Grille, -999) :- victoireJoueur(o, Grille), !.
...
ajouterOrphelin(Orphelin) :- 
    retract(orphelins(Orphelins)), 
    append([Orphelin], Orphelins, NouveauxOrphelins), 
    assert(orphelins(NouveauxOrphelins)).

initialiserOrphelins :- 
    retractall(orphelins(_)),
    assert(orphelins([])).
